...
with col_sql2:
    ...
    st.markdown("#### 💡 인사이트")
    st.info(
        "대부분의 자치구에서 실내 시설이 80% 이상을 차지하며, "
        "실외시설의 비율과 공원 수 간의 뚜렷한 상관관계는 존재하지 않습니다. "
        "특히 용산구와 종로구의 실외시설 비율과 공원 수를 비교하면 분명하게 드러납니다. "
        "이는 하나의 기준으로는 자치구의 반려시설 성향을 파악하지 어렵다는 것을 의미합니다."
    )
...
with col_sql3:
    ...
    st.markdown("#### 💡 인사이트")
    st.info(
        "**강북구·용산구·강남구**는 반려화율(등록동물/인구)이 높아 "
        "반려동물 친화적 주거 문화가 정착된 지역으로 기대할 수 있습니다. "
